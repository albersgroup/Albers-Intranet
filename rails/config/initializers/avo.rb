...
Avo.configure do |config|
  ...
  ## == Authentication ==
  # Avo controllers don't inherit the host ApplicationController, so resolve the
  # current user from the session directly (runs in the Avo controller context).
  config.current_user_method do
    User.find_by(id: session[:user_id]) if session[:user_id]
  end
  config.authenticate_with do
    unless _current_user&.admin? || _current_user&.division_admin?
      redirect_to main_app.login_path, alert: "Admin access required."
    end
  end
