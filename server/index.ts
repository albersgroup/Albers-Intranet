...
pgPool.on('error', (err) => {
  console.error('Database pool error (session):', err.message);
});

// Session configuration with PostgreSQL backing
app.use(session({
  name: 'portal.sid', // Custom cookie name to avoid conflicts with BI app
  store: new PgSession({
    pool: pgPool,
    tableName: 'session',
  ...
  cookie: {
    ...
    sameSite: 'lax', // Allow cookies on top-level navigation (important for SSO)
  }
}));
